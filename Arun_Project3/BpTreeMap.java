...
//-----------------------------------------------------------------------------------
     ...
     * @author Sanjana Arun - assisted 
 *     @author Mukta Deshmukh - assisted
     *
     * @param key  the key to insert
        ...
        if (n.isLeaf) {                                               // handle LEAF node level
            ...
            if (rt != null) {
                ...
                root = new Node (root, rt.key[0], rt);                // make a new root
                                                               ...
                                                               // handle INTERNAL node level
            rt = insert (key, ref, (Node) n.ref[n.find (key)]);       // recursive call to insert
            if (rt == null) {
                return rt;
            } //if
            //copy value from leaf
            if (rt.isLeaf) {
                rt = addI(n, rt.key[0], rt);
            } else {
                Node stopper = rt;
                //find value from middle to push onto this node
                while (!stopper.isLeaf) {
                    stopper = (Node) stopper.ref[0];
                } //while
                rt = addI(n, stopper.key[0], rt);
            } //if
            //handles root
            if (rt != null) {
                if (n != root) return rt;
                root = new Node (root, root.key[HALF], rt);
            } //if
        } // if
...
//-----------------------------------------------------------------------------------
    ...
    public static void main (String [] args)
    {
        ...
        if (RANDOMLY) {
            ...
            for (var i = 1; i <= totalKeys; i += 1) bpTree.put (rng.nextInt (2 * totalKeys), i * i);
        } else {
            for (var i = 1; i <= totalKeys; i += 1) bpTree.put (i, i * i);
        } // if
